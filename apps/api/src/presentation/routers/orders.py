...
async def trigger_manual_sync():
    """Executa sincronismo sob demanda da API do BaseLinker gravando tudo no Banco"""
    stats = await sync_service.sync_all_real_data()
    return {"message": "Sincronização com BaseLinker realizada com sucesso!", "stats": stats}
...
async def change_order_status(order_id: str, payload: Dict[str, Any]):
    """Altera o status do pedido localmente e propaga via API BaseLinker"""
    ...
    return {"status": "SUCCESS", "order_id": order_id, "new_status": new_status_name}
